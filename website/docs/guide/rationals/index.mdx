...
The purpose of configurability is to control the behaviour of components and the relationships between them. The component writer includes as many different behaviours as possible but cannot predict exactly how a particular component will be used. Therefore, when an application uses a component, there must be a way to specify the desired behaviour.
