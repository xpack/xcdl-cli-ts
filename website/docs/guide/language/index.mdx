...
Like a `cdlPackage`, a `cdlComponents` object has a name and a body. The body
...
`cdlPackage`, `cdlComponents`, `cdlOptions` and `cdlInterfaces`. These objects
...
`cdlComponents`, `cdlOptions` and `cdlInterfaces` define maps of named objects:
...
All objects have a mandatory **name** and, within their body, a list of
...
the names more manageable in size.

The hierarchy serves two purposes. First, it allows objects to be controlled en
...
Each package has one top-level xCDL file, `xcdl*.json`. There should be a single
top-level `cdlPackage` object per package.
...
Many of the properties can be used in any of the `cdlPackage`, `cdlComponents`,
`cdlOptions` or `cdlInterfaces` objects. However, some properties are more
specific. The `includeCDLs` property is only relevant to `cdlPackage` and
...
The `includeCDLs` property can only be used in the body of a `cdlPackage` or a
...
xCDL objects have internal names, used to refer objects in the configuration
...
Using the JSON syntax, nodes names are represented as JSON member names and node
bodies as JSON objects.
