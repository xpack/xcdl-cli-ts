...
As a design decision, xCDL objects must map 1:1 to JavaScript objects. This means they should have properties whose names match the JavaScript naming convention and have values of type string, number, object, or array of objects (not necessarily of the same type).
...
Each node has an internal name used to refer to objects in the hierarchy. Names are short strings and must follow these rules:
...
Using JSON syntax, nodes are represented as JSON member names with object values.
...
Node names are used to refer to objects and to compose permalinks in documentation websites.

When used in a GUI, the node `display` property is shown to identify the node.

Internally, nodes must also have a property defining the node type, allowing a GUI to associate different icons with different nodes.
...
If a node has children, they are grouped as one or more containers.
...
Components can include additional configuration objects. For example, a device driver might have options to control its specific behavior, which become irrelevant if the driver is disabled. Generally, options and components exist in a hierarchy, where any component or package can contain options specific to that component and further sub-components.
