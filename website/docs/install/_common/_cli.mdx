...
```txt
npm install -location=global npm@latest --verbose
```
...
```sh
npm install --location=global xcdl@latest --verbose
```
...
`npm install --location=global xcdl@${customField('releaseVersion')} --verbose
`} </CodeBlock>
