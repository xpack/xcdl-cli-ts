...
If missing, it is assumed that the current object does not implement any
interface.
...
It is possible for an object to implement a given interface multiple times, but
only distinct implementors are counted.
