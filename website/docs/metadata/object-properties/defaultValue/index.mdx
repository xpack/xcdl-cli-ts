...
- `none` - these objects have no value, and the `defaultValue` is not applicable
- `bool` - enabled objects have the value `true` and non enabled objects have
  the value `false`
- `int` - enabled objects have the value 1 and non enabled objects have the
  value 0
- `float` - enabled objects have the value 1.0 and non enabled objects have the
  value 0.0
- `string` - enabled objects have the value "1" and non enabled objects have the
  value "0"
...
called Ordinary Expressions in [xCDL packages](/docs/guide/packages) for the
syntactic details. In many cases a simple constant value will suffice.

However, it is also possible for an object's default value to depend on other
objects. For example, the common HAL package provides some support functions
needed by the kernel, but are unlikely to be useful if the kernel is not being
used.
...
```json
{
  "cdlOptions": [
    {
      "id": "hal.debug.gdb.thread-support",
